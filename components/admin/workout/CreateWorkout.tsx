// // test code for drag and drop option 
// 'use client';
...
//     const handleDragStart = (e: React.DragEvent<HTMLDivElement>, movement: Movement) => {
//         e.dataTransfer.effectAllowed = 'copy';
//         e.dataTransfer.setData('application/json', JSON.stringify(movement));
//     };
...
//     const handleDropOnCategory = (e: React.DragEvent<HTMLDivElement>, categoryName: string) => {
//         e.preventDefault();
//         setDragOverCategory(null);

//         try {
//             const movementData = e.dataTransfer.getData('application/json');
//             const movement: Movement = JSON.parse(movementData);

//             const currentMovements = categoryMovements[categoryName] || [];
//             const maxAllowed = getMaxMovementsForCategory(categoryName);

//             // Check if movement already exists
//             if (currentMovements.some((m) => m.id === movement.id)) {
//                 toast.error('This movement is already in this category');
//                 return;
//             }

//             // Check if we've reached the limit
//             if (currentMovements.length >= maxAllowed) {
//                 toast.error(`Maximum ${maxAllowed} movements allowed for ${categoryName}`);
//                 return;
//             }

//             // Add the movement
//             const newMovement: SelectedMovement = {
//                 ...movement,
//                 order: currentMovements.length + 1,
//             };

//             setCategoryMovements((prev) => ({
//                 ...prev,
//                 [categoryName]: [...(prev[categoryName] || []), newMovement],
//             }));
            
//             toast.success(`${movement.name.length > 20 ? movement.name.substring(0, 20) + '...' : movement.name} added to ${categoryName}`);
//         } catch (error) {
...
//             <div
//                 key={categoryName}
//                 className={`class-type-box ${isDragOver ? 'drag-over' : ''}`}
...
//                         <p className="empty-text">No movements selected - drag here to add</p>
//                     ) : (
//                         <>
...
//                                 Cancel
//                             </button>
//                             <button type="button" className="btn create" onClick={saveSelection}>
...
// workout create form with static categories
// 'use client';
