...
const Workouts: React.FC = () => {
    ...
    const getExercises = (movements: WorkoutMovement[]): string => {
        const firstThree = movements
            .slice(0, 3)
            .map(wm => wm.movements.name);
                    ...
                    <div className="workouts-grid">
                        {currentWorkouts.map((workout) => (
                            <div key={workout.id} className="workout-card" onClick={() => handleCardClick(workout.id)}>
                                ...
                                <div className="workout-duration">{formatDuration(workout.total_duration)}</div>
                                <div className="workout-difficulty">{getDifficulty(workout.total_duration)}</div>
